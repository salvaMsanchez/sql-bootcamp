...
CREATE SCHEMA gestionFlota;

-- +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
-- Importación de los datos no normalizados del supuesto cliente
-- +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
CREATE TABLE gestionFlota.coches (
...
INSERT INTO gestionFlota.coches (matricula,grupo,marca,modelo,fecha_compra,color,aseguradora,n_poliza,fecha_alta_seguro,importe_revision,moneda,kms_revision,fecha_revision,kms_totales) VALUES
...
-- Limpieza de matrículas repetidas en modelo de datos no normalizado. En un caso real no encontraríamos dicha repetición, por ello las elimino
delete from gestionFlota.coches where matricula in ('2066BYF', '7764GTD', '6335BFK', '3230KTX', '4916HJG', '0326HRM', '9729KXJ', '3187KKM', '1323DQL', '7792CKF', '5648JTZ', '2438GSV', '7221BJG', '3242GQG', '7466DMG', '5047FJK', '5188DWK', '5022JZD', '3306GYM', '2428HYB', '6756GXW', '5426HDG', '4221JXR', '0922JVF', '6743DYG', '2835JQN', '2430FDP', '6642GZP', '9209KGR', '7561CND', '8177JPM', '7938HXH', '0366CKQ');

-- +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
-- Construcción del modelo de datos normalizado según nuestro diagrama Entidad-Relación
-- +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

CREATE TABLE gestionFlota.grupoEmpresarial (
	id SERIAL primary key not null,
  	nombre VARCHAR(50) not null
);

CREATE TABLE gestionFlota.marca (
	id SERIAL primary key not null,
  	nombre VARCHAR(50) not null,
  	id_grupo_empresarial INT not null references gestionFlota.grupoEmpresarial(id)
);

CREATE TABLE gestionFlota.modelo (
	nombre VARCHAR(50) primary key not null,
  	id_marca INT not null references gestionFlota.marca(id)
);

CREATE TABLE gestionFlota.companiaAseguradora (
	id SERIAL primary key not null,
  	nombre VARCHAR(50) not null
);

CREATE TABLE gestionFlota.color (
	id SERIAL primary key not null,
  	nombre VARCHAR(50) not null
);
...
CREATE TABLE gestionFlota.divisa (
	nombre VARCHAR(50) primary key not null,
  	abreviatura VARCHAR(3) not null DEFAULT '???',
  	simbolo VARCHAR(5) not null DEFAULT '?????'
);

CREATE TABLE gestionFlota.revision (
	id SERIAL primary key not null,
  	matricula VARCHAR(9) not null references gestionFlota.coche(matricula),
  	kilometros INT not null,
  	fecha DATE not null,
  	importe NUMERIC(10, 2) not null,
  	divisa VARCHAR(50) not null references gestionFlota.divisa(nombre)
);
...
-- MAPEO TABLA DIVISA
INSERT INTO gestionFlota.divisa (nombre)
SELECT DISTINCT moneda FROM gestionFlota.coches;

-- adición de abreviatura y símbolo para la divisa Euro
UPDATE gestionFlota.divisa
SET abreviatura = 'EUR', simbolo = '€'
WHERE nombre = 'Euro';

-- adición de abreviatura y símbolo para la divisa Dólar
UPDATE gestionFlota.divisa
SET abreviatura = 'USD', simbolo = 'US$'
WHERE nombre = 'Dolar';

-- adición de abreviatura y símbolo para la divisa Peso Colombiano
UPDATE gestionFlota.divisa
SET abreviatura = 'COP', simbolo = 'CO$'
WHERE nombre = 'Peso Colombiano';

-- adición de abreviatura y símbolo para la divisa Peso Mexicano
UPDATE gestionFlota.divisa
SET abreviatura = 'MXN', simbolo = 'MX$'
WHERE nombre = 'Peso Mexicano';

-- MAPEO TABLA COMPANIAASEGURADORA
INSERT INTO gestionFlota.companiaAseguradora (nombre)
SELECT DISTINCT aseguradora FROM gestionFlota.coches;

-- MAPEO TABLA GRUPOEMPRESARIAL
INSERT INTO gestionFlota.grupoEmpresarial (nombre)
SELECT DISTINCT grupo FROM gestionFlota.coches;

-- MAPEO TABLA COLOR
INSERT INTO gestionFlota.color (nombre)
SELECT DISTINCT color FROM gestionFlota.coches;

-- MAPEO TABLA MARCA
INSERT INTO gestionFlota.marca (nombre, id_grupo_empresarial)
SELECT DISTINCT gestionFlota.coches.marca, gestionFlota.grupoEmpresarial.id
FROM gestionFlota.coches
INNER JOIN gestionFlota.grupoEmpresarial
ON gestionFlota.coches.grupo = gestionFlota.grupoEmpresarial.nombre
ORDER BY gestionFlota.grupoEmpresarial.id;

-- MAPEO TABLA MODELO
INSERT INTO gestionFlota.modelo (nombre, id_marca)
SELECT DISTINCT gestionFlota.coches.modelo, gestionFlota.marca.id
FROM gestionFlota.coches
INNER JOIN gestionFlota.marca
ON gestionFlota.coches.marca = gestionFlota.marca.nombre
ORDER BY gestionFlota.marca.id;
...
-- MAPEO TABLA REVISION
INSERT INTO gestionFlota.revision (matricula, kilometros, fecha, importe, divisa)
SELECT gestionFlota.coche.matricula, kms_revision, fecha_revision, importe_revision, gestionFlota.divisa.nombre
FROM gestionFlota.coches
INNER JOIN gestionFlota.coche
ON gestionFlota.coches.matricula = gestionFlota.coche.matricula
INNER JOIN gestionFlota.divisa
ON gestionFlota.coches.moneda = gestionFlota.divisa.nombre;

-- Eliminación de la tabla que alberga el modelo no normalizado dado por cliente
DROP TABLE gestionFlota.coches;

-- ++++++++++++++++
-- + Consulta SQL +
-- ++++++++++++++++
--
-- Extraer el listado de coches que hay mostrando:
--
...
SELECT 
modelo, 
gestionFlota.marca.nombre AS marca, 
gestionFlota.grupoEmpresarial.nombre AS grupo_empresarial, 
fecha_compra, 
matricula,
